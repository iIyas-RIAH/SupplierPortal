...
<head>
    ...
    <title>ONHYM Supplier Portal | Esapace resultats</title>
...
<body>
    ...
    $depense = mysqli_fetch_row(mysqli_query($conn, "SELECT * FROM depense WHERE NUMDEPENSE = '$idDepense'"));
    $entreprise = mysqli_fetch_row(mysqli_query($conn, "SELECT NOM FROM entreprise WHERE entreprise.IDFOURNISSEUR = '$depense[0]'"));
    ...
    <div class="flex h-screen bg-gray-50" :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full">
                    ...
                    <div class="bg-white px-8 shadow-md rounded-md py-5">
                        <div>
                            <div class="flex items-center space-x-2 text-xl font-semibold text-gray-900 leading-8 px-4 py-3">
                                ...
                                <span class="px-1">Dépense</span>
                            </div>

                            <form class="grid grid-cols-2 gap-x-12 gap-y-6 pl-4" action="../../Service/EditDepenseService.php" method="post" id="form">
                                <div>
                                    <h4 class="py-4">Type Dépence</h4>
                                    <input class="w-11/12 rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Type_Depence" value="<?php echo $depense[3]; ?>">
                                </div>

                                <div>
                                    <h4 class="py-4">Num Depense</h4>
                                    <input class="w-11/12 rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Num_Depense" value="<?php echo $depense[0]; ?>" readonly>
                                </div>

                                <div>
                                    <h4 class="py-4">Fournisseur</h4>
                                    <a href="InfoFournisseur.php?idFournisseur=<?php echo $depense[1]; ?>">
                                        <input class="w-11/12  rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Fournisseur" value="<?php echo $entreprise[0]; ?>" readonly>
                                    </a>
                                </div>

                                <div>
                                    <h4 class="py-4">Libelle</h4>
                                    <input class="w-11/12 rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Libelle" value="<?php echo $depense[2]; ?>">
                                </div>

                                <div>
                                    <h4 class="py-4">Etat</h4>
                                    <input class="w-11/12 rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Etat_Depense" value="<?php echo $depense[4]; ?>">
                                </div>

                                <div>
                                    <h4 class="py-4">Date Début</h4>
                                    <input class="w-11/12  rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="date" name="Date_Debut" value="<?php echo $depense[5]; ?>" readonly>
                                </div>

                                <div>
                                    <h4 class="py-4">Date Fin</h4>
                                    <input class="w-11/12  rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="date" name="Date_Fin" value="<?php echo $depense[6]; ?>" readonly>
                                </div>

                                <div>
                                    <h4 class="py-4">MONTANT</h4>
                                    <input class="w-11/12  rounded-lg border-2 border-black px-3 text-gray-700 h-10" type="text" name="Montant" value="<?php echo $depense[7]; ?>" readonly>
                                </div>

                            </form>
                            <div class="flex justify-center pt-5">
                                <button class="block w-5/12 px-4 py-2 my-4 mx-4 text-lg font-medium leading-5 text-center text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-purple" onclick="toggleModal('modal-id')">
                                    Enregistrer
                                </button>

                                <button class="block w-5/12 px-4 py-2 my-4 mx-4 text-lg font-medium leading-5 text-center text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-lg active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-purple" onclick="toggleModal('modal-suppression-depense')">
                                    Supprimer
                                </button>
                            </div>
                        </div>
            ...
            <?php include '../components/suppModalDepense.php'; ?>
