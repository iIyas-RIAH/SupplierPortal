...
<body>
    ...
    <div class="flex h-screen bg-gray-50" :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full">
            ...
            <header class="z-10 py-4 bg-white shadow-md">
                <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600">
                    ...
                    <div class="flex justify-center flex-1 lg:mr-32">
                        <form class="relative w-full max-w-xl mr-6 focus-within:text-purple-500" id="search" action="SearchDepense.php" method="post">
            ...
            <main class="h-max bg-gray-50">

                <div class="container px-6 mx-auto">
                    <h3 class="text-xl mt-4">
                        Bienvenue <?php echo $resultat[1] . " " . $resultat[2]; ?>
                    </h3>

                    <h2 class="my-4 text-3xl font-semibold text-gray-800">
                        Mon Dashboard
                    </h2>
                    ...
                    <div class="pb-14 pt-3 px-6" style="width: 99%;">
                        ...
                        <div class="bg-white px-4 shadow-md rounded-md py-4">
                            <div class="w-full h-max px-6 py-4">
                                    ...
                                    <div class="w-full overflow-x-auto">
                                        <table class="w-full whitespace-no-wrap">
                                            ...
                                            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                                ...
                                                <?php
                                                $results = mysqli_query($conn, "SELECT fournisseur.IDFOURNISSEUR, fournisseur.NOM, fournisseur.PRENOM, entreprise.NOM  FROM fournisseur, entreprise WHERE VALIDÉ = false and fournisseur.IDFOURNISSEUR = entreprise.IDFOURNISSEUR");

                                                while ($ligne = mysqli_fetch_row($results)) {
                                                ?>
                                                    <tr class="text-gray-700 dark:text-gray-400" onclick="window.location='InfoFournisseur.php?idFournisseur=<?php echo $ligne[0]; ?>';">
                                                        <td class="px-4 py-3 text-sm font-semibold">
                                                            <?php echo $ligne[0]; ?>
                                                        </td>
                                                        <td class="px-4 py-3 text-sm font-semibold">
...
<?php
$sql = "SELECT DATE_FORMAT(DATEDEPOSE, '%m') AS MOIS, COUNT(NUMFACTURE) AS Count,
        COUNT(IF(ETATFACTURE != 'Déposée' AND ETATFACTURE != 'Rejetée', 1, NULL)) AS Validé,
        COUNT(IF(ETATFACTURE = 'Rejetée', 1, NULL)) AS Rejeté FROM facture GROUP BY MONTH(DATEDEPOSE), YEAR(DATEDEPOSE)";
...
<script>
    ...
    const FM = <?php echo json_encode($Count); ?>;
    const Validé = <?php echo json_encode($Validé); ?>;
    const Rejeté = <?php echo json_encode($Rejeté); ?>;
    ...
    window.onload = function() {
        var ctx = document.getElementById("factureChart").getContext("2d");
    ...
    document.getElementById("searchField").keydown(function(e) {
        if (e.keyCode == 13) {
            document.getElementsByName("search").submit();
        }
    });
</script>
